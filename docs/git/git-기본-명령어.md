...
- **`git log`**
    ...
    **저장소의 커밋 기록을 확인**합니다. 각 커밋의 메시지, ID, 작성자 등의 정보가 표시됩니다.
    ...
    _예시:_ `git log --oneline` (한 줄씩 요약 표시)
...
- **`git branch`**
    ...
    _예시:_ `git branch feature-branch` (브랜치 생성)
    _예시:_ `git branch` (브랜치 조회)
...
- **`git stash`**
    ...
    _예시:_ `git stash` (변경 사항 저장) / `git stash pop` (저장한 변경 사항 복구)
...
- **`git reset --hard <커밋ID>`**
    ...
    주의: 이 명령은 되돌린 변경 사항을 **복구할 수 없으므로** 신중하게 사용하세요.


    _예시:_ `git reset --hard HEAD~1` (이전 커밋으로 되돌리기)
